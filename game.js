...
const punchRange = 45; const kickRange = 65; const grabRange = 40;
const punchDuration = 155; const kickDuration = 345; const jumpKickDuration = 275; const kneeDuration = 195;
...
function drawShadow(x, y, width, height) { const shadowY=platformBottom+8; const cBY=y+height; const rY=clamp((cBY-platformTop)/platformDepth,0,1); const sS=0.7+rY*0.3; const sO=0.2+rY*0.25; ctx.fillStyle=`rgba(0,0,0,${sO})`; ctx.beginPath(); ctx.ellipse(x+width/2,shadowY,(width/2.8)*sS,6*sS,0,0,Math.PI*2); ctx.fill(); }
...
class Character {
    ...
    updateState(deltaTime) {
        ...
        if (this.isOnPlatform) {
            ...
            this.vy = 0; // Reset vertical velocity when on platform
...
// --- Player Class ---
class Player extends Character { constructor(x,y){super(x,y,45,85,'#e0c0a0',100);this.shirtColor='#0077cc';this.pantsColor='#333344';this.skinColor=this.baseColor;this.hairStyle='short';this.hairColor='#5b3a1a';this.isGrappling=false;this.grappledEnemy=null;} update(enemies,deltaTime){if(this.isDead||this.isFalling||this.isKnockedDown){this.isWalking=false;if(this.isGrappling&&this.grappledEnemy){this.grappledEnemy.isGrabbed=false;this.grappledEnemy.grabbedBy=null;} this.isGrappling=false;this.grappledEnemy=null;this.updateState(deltaTime);return;} if(this.isHit){this.isWalking=false;this.updateState(deltaTime);return;} if(this.isGrappling){this.isWalking=false;if(!this.grappledEnemy||this.grappledEnemy.hp<=0||this.grappledEnemy.isDead||this.grappledEnemy.isKnockedDown||this.grappledEnemy.isFalling||!this.grappledEnemy.isOnPlatform||this.grappledEnemy.isGrabbed===false){if(this.grappledEnemy&&this.grappledEnemy.grabbedBy===this){this.grappledEnemy.isGrabbed=false;this.grappledEnemy.grabbedBy=null;} this.isGrappling=false;this.grappledEnemy=null;this.attackCooldownTimer=150;}else{this.grappledEnemy.x=this.x+(this.facingRight?this.width*0.7:-this.grappledEnemy.width*0.7);this.grappledEnemy.y=this.y+(this.height-this.grappledEnemy.height)/2;this.grappledEnemy.vx=0;this.grappledEnemy.vy=0;this.grappledEnemy.isGrounded=this.isGrounded;this.grappledEnemy.facingRight=!this.facingRight; if((keysPressed['KeyZ']||keysPressed['KeyN'])&&this.attackCooldownTimer<=0){this.attack('knee');} else if(keysPressed['Space']&&this.attackCooldownTimer<=0){this.attack('throw');} this.updateState(deltaTime);return;}} let dx=0,dy=0;this.isWalking=false; const dtFactor=deltaTime/16.67; if(!this.isAttacking){if(keysPressed['ArrowLeft']||keysPressed['KeyA']){dx=-playerSpeed;} if(keysPressed['ArrowRight']||keysPressed['KeyD']){dx=playerSpeed;} if(keysPressed['ArrowUp']||keysPressed['KeyW']){dy=-playerSpeed*0.7;} if(keysPressed['ArrowDown']||keysPressed['KeyS']){dy=playerSpeed*0.7;} if(dx!==0||dy!==0)this.isWalking=true; else this.isWalking=false; if(dx!==0)this.facingRight=dx>0;} if(dx!==0&&dy!==0){dx*=0.707;dy*=0.707;} this.x+=dx*dtFactor; this.y+=dy*dtFactor; this.updateState(deltaTime); if(this.attackCooldownTimer<=0&&!this.isHit){if(keysPressed['Space']&&this.isGrounded&&!this.isAttacking){this.vy=jumpPower;this.isGrounded=false;this.isWalking=false;} else if(keysPressed['KeyZ']||keysPressed['KeyN']){this.attack('punch');} else if((keysPressed['KeyX']||keysPressed['KeyM'])&&this.isGrounded){this.attack('kick');} else if((keysPressed['KeyX']||keysPressed['KeyM'])&&!this.isGrounded&&!this.isAttacking){this.attack('jumpKick');} else if(keysPressed['KeyC']&&!this.isAttacking&&this.isGrounded){this.attemptGrab(enemies);}} playerHpElement.textContent=Math.max(0,Math.ceil(this.hp)); if(this.hp<=0&&gameRunning&&!this.isFalling&&!this.isDead){gameOver('Defeated!');}} attack(type){if(this.isAttacking||this.attackCooldownTimer>0)return; let duration=0,damage=0,range=0; let attackHeight=this.height*0.4,attackYOffset=this.height*0.3; this.isWalking=false; switch(type){case 'punch':duration=punchDuration;damage=punchDamage;range=punchRange;break; case 'kick':duration=kickDuration;damage=kickDamage;range=kickRange;attackYOffset=this.height*0.6;break; case 'jumpKick':duration=jumpKickDuration;damage=jumpKickDamage;range=kickRange*1.1; attackYOffset=this.height*0.5;attackHeight=this.height*0.4; this.vx+=(this.facingRight?2.5:-2.5); this.vy=Math.min(this.vy,1.5); break; case 'knee':if(!this.isGrappling||!this.grappledEnemy)return; duration=kneeDuration;damage=kneeDamage; this.grappledEnemy.takeDamage(damage,this,type); break; case 'throw':if(!this.isGrappling||!this.grappledEnemy)return; duration=150;damage=throwDamage; const throwForceX=this.facingRight?11:-11;const throwForceY=-7.5; this.grappledEnemy.isGrabbed=false;this.grappledEnemy.grabbedBy=null; this.grappledEnemy.vy=throwForceY;this.grappledEnemy.vx=throwForceX; this.grappledEnemy.takeDamage(damage,this,type); this.isGrappling=false;this.grappledEnemy=null; break; default:console.warn("Unknown attack type:",type); return;} this.isAttacking=true; this.attackType=type; this.attackTimer=duration; if(type==='punch'||type==='kick'||type==='jumpKick'){const aXC=this.x+this.width/2+(this.facingRight?this.width/4:-this.width/4); const aHX=this.facingRight?aXC:aXC-range; const aHW=range; const aHY=this.y+attackYOffset-attackHeight/2; const aHH=attackHeight; console.log(`Attack: ${type}, Y: ${aHY.toFixed(1)}, H: ${aHH.toFixed(1)}`); enemies.forEach(enemy=>{if(enemy&&enemy.hp>0&&!enemy.isDead&&!enemy.isGrabbed&&!enemy.isKnockedDown&&!enemy.isFalling&&enemy.isOnPlatform){if(aHX<enemy.x+enemy.width&&aHX+aHW>enemy.x&&aHY<enemy.y+enemy.height&&aHY+aHH>enemy.y){const eCX=enemy.x+enemy.width/2;const pCX=this.x+this.width/2; const hOCS=(this.facingRight&&eCX>pCX-this.width*0.2)||(!this.facingRight&&eCX<pCX+this.width*0.2); if(hOCS){console.log(`HITBOX COLLISION! Type: ${type}, Enemy: ${enemy.constructor.name}, EnemyY: ${enemy.y.toFixed(1)}, EnemyH: ${enemy.height.toFixed(1)}`); const hit=enemy.takeDamage(damage,this,type); console.log(`-> Damage applied? ${hit}`);}}}});}} attemptGrab(enemies){if(this.isAttacking||this.isGrappling||!this.isGrounded)return; const gCX=this.x+(this.facingRight?this.width*0.5:0); const gCW=this.width*0.5+grabRange; const gCY=this.y+this.height*0.1; const gCH=this.height*0.8; let target=null; let closestDSq=(grabRange*1.5)**2; enemies.forEach(e=>{if(e&&e.hp>0&&!e.isDead&&!e.isGrabbed&&!e.isKnockedDown&&!e.isFalling&&e.isOnPlatform){if(e.y+e.height>gCY&&e.y<gCY+gCH){const eCX=e.x+e.width/2;const pCX=this.x+this.width/2; const dx=eCX-pCX;const dSq=dx*dx; const oCS=(this.facingRight&&dx>0)||(!this.facingRight&&dx<0); const wR=Math.abs(dx)<(this.width/2+e.width/2+grabRange); if(oCS&&wR&&dSq<closestDSq){closestDSq=dSq;target=e;}}}}); if(target){this.isGrappling=true;this.grappledEnemy=target;target.isGrabbed=true;target.grabbedBy=this;target.isHit=false;target.comboHits=0;this.isWalking=false;this.attackCooldownTimer=250;}} }
// --- Enemy Classes ---
...
function spawnEnemy(){let spawnX,spawnY,isDoorSpawn=false; const doorSpawnX=200+35; const doorExitX=doorSpawnX+50; spawnY=platformTop+platformDepth*0.33+Math.random()*(platformDepth*0.34); if(Math.random()<0.25){spawnX=doorSpawnX-20; isDoorSpawn=true;}else{spawnX=Math.random()<0.5?-100:canvas.width+100;} let newEnemy; const enemyType=getRandomInt(1,3); switch(enemyType){case 1:newEnemy=new GruntEnemy(spawnX,spawnY);break; case 2:newEnemy=new FastEnemy(spawnX,spawnY);break; case 3:newEnemy=new FemaleEnemy(spawnX,spawnY);break; default:newEnemy=new GruntEnemy(spawnX,spawnY);} if(isDoorSpawn){newEnemy.state='exitingDoor'; newEnemy.exitDoorTargetX=doorExitX; newEnemy.facingRight=true;}else{const dx=newEnemy.x-player.x, dy=newEnemy.y-player.y; if(dx*dx+dy*dy<150*150){newEnemy.x=-spawnX;}} enemies.push(newEnemy); enemiesSpawnedCount++; console.log(`Spawned enemy #${enemiesSpawnedCount}/${totalEnemiesToSpawn}. Door: ${isDoorSpawn}. Alive: ${enemies.filter(e=>!e.isDead).length}`);}
